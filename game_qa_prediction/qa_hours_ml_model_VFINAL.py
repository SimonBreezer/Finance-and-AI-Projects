...
# Convert datetime columns to days from today
reference_date = datetime(2025, 2, 9)  # Today's date as per the provided information

for col in ['Alpha_WSR', 'Beta_WSR', 'FormatQASubmission_WSR']:
    X[col] = (reference_date - X[col]).dt.days

# Identify numeric features now that datetime has been converted
numeric_features = X.columns

# Preprocessor for numeric features
numeric_transformer = StandardScaler()

# Since all features are now numeric, we can apply StandardScaler to all
...
print("Linear Regression - Cross-validation R2 Score:", cv_scores_lr.mean())
...
print("Best Random Forest - MSE:", mean_squared_error(y_test, y_pred_rf_best))
print("Best Random Forest - R2 Score:", r2_score(y_test, y_pred_rf_best))

# Cross-validation for Random Forest
cv_scores_rf = cross_val_score(best_rf, X_encoded, y, cv=5, scoring='r2')
print("Random Forest - Cross-validation R2 Score:", cv_scores_rf.mean())
